 ...
 class JsonArray : List<object>
    {
        ...
        static public int toString(IntPtr l)
        {
            try
            {
                JsonArray self = (JsonArray)LuaObject.checkSelf(l);
                string str = self.ToString();
                LuaObject.pushValue(l, true);
                LuaObject.pushValue(l, str);
                return 2;
            }
            catch (Exception e)
            {
 ...
 IDictionary<string, object>
    {
        ...
        private static Dictionary<string, string> _strTostr = new Dictionary<string,string>();
        string Str2Str(string strKey)
        {
            string val = "";
            if (_strTostr.TryGetValue(strKey, out val))
            {
                return val;
            }
            // if we can not find key,then we will create one
            //val = _KeyCount++;
            _strTostr.Add(strKey, strKey);
            return strKey;
        }
        /// <summary>
		...
		public JsonObject link(string colName)
		{
			return _members["_" + colName] as JsonObject;
		}
		public string str(string colName)
		{
			return get<string>(colName);
		}
		public int num(string colName)
		{
		...
		public T get<T>(string colName)
		{
			...
			if (_members.TryGetValue(colName,out val))
			{
				if (val is T) return (T)val;
				if (typeof(T) == typeof(Int32)) {
					object nVal = Convert.ToInt32(val);
					return (T)nVal;
				}
				if (typeof(T) == typeof(Int64)) {
					object nVal = Convert.ToInt64(val);
					return (T)nVal;
				}
				if (typeof(T) == typeof(Int16)) {
					object nVal = Convert.ToInt16(val);
					return (T)nVal;
				}
			}
			return default(T);
        ...
        {
            _members.Add(Str2Str(key), value);
        }
        ...
        public object this[string key]
        {
            ...
            set { _members[Str2Str(key)] = value; }
        }
        ...
        {
            _members.Add(Str2Str(item.Key), item.Value);
        }
        ...
        static public int toString(IntPtr l)
        {
            try
            {
                JsonObject self = (JsonObject)LuaObject.checkSelf(l);
