...
namespace TinyJSON
{
	...
	public struct JSON
	{
        ...
        private static JSON mJSON = new JSON();
        static public Node parse(string input)
        {
            return mJSON.Load(input);
        }

        private static Printer mPrinter;
        static public string stringify(Node json)
        {
            if (mPrinter == null) mPrinter = new Printer();
            return mPrinter.String(json);
        }
