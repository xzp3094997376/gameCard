...
namespace PathologicalGames
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        private void Update()
        {
			for(int i=0; i < this._prefabPools.Count; i++)
			{
				this._prefabPools[i].RunningCullDelay();
			}
        }
        ...
        public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot)
        {
            ...
            for (int i = 0; i < this._prefabPools.Count; i++)
            {
                ...
                {
					//add chiuan
					//check if this pool not exist,need removed and create a new pool.
					if(this._prefabPools[i].totalCount <= 0 )
					{
						//this.prefabs._Remove(this._prefabPools[i].prefab.name);
						//this._prefabPools.RemoveAt(i);
                        #if UNITY_EDITOR
                            Debug.LogWarning("SpawnPool"+"["+poolName+"]" + " Spawn " + prefab.name + " is pool empty. try instance new prefab");
                        #endif
                        //return null;
						//break;
					}
                    ...
                    if (inst == null){
						...
						this._prefabPools.RemoveAt(i);
						#if UNITY_EDITOR
						Debug.LogWarning("SpawnPool SpawnInstance " + prefab.name + " is pool empty.");
						#endif
						break;
					}
                    ...
                    // add by chiuan SendMessage  -  BroadcastMessage
                    //inst.gameObject.SetActive(true);
	                inst.gameObject.SendMessage(
						"OnSpawned",
						this,
    ...
    public class PrefabPool
    {
		...
		//add by chiuan
		private bool m_culling = false;
		private float m_delayCullCounter = 0f;
		public bool isCull{
			set{
				m_culling = value;
				m_delayCullCounter = 0f;
			}
		}
		...
		public int spawnedCount{
			get{
				int _t = 0;
				for(int i=0; this._spawned != null && i < this._spawned.Count; i++)
				{
					if(this._spawned[i] != null) _t++;
				}
				return _t;
			}
		}
		...
		public int despawnedCount{
			get{
				int _t = 0;
				for(int i=0; this._despawned != null && i < this._despawned.Count; i++)
				{
					if(this._despawned[i] != null) _t++;
				}
				return _t;
			}
		}
		...
		public void RunningCullDelay()
		{
			if(m_culling == false) return;

			if(m_delayCullCounter >= cullDelay)
			{
				CullDespawned();
			}
			else
			{
				m_delayCullCounter += Time.deltaTime;
			}
		}
            ...
            while (this.totalCount > this.cullAbove)
            {
                ...
                for (int i = 0; i < this.cullMaxPerPass; i++)
                {
                    ...
                    if (this._despawned.Count > 0)
                    {
						...
						if(inst != null) MonoBehaviour.Destroy(inst.gameObject);
        ...
        internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
        {
			...
			if (despawnedCount == 0 ||(this._despawned != null && this._despawned.Count > 0 && this._despawned[0] == null))
            {
			...
			isCull = false;
			if(inst != null) PoolManagerUtils.SetActive(inst.gameObject, true);
    ...
    public class PrefabsDict : IDictionary<string, Transform>
    {
        ...
        {
            //add by chiuan
            bool t_exist = this._prefabs.ContainsKey(prefabName);
            if(t_exist && _prefabs[prefabName] == null)
            {
                t_exist = false;
                _Remove(prefabName);

                #if UNITY_EDITOR
                    Debug.LogError("SpawnPool ContainsKey is exist prefabName but transform is null.");
                #endif
            }
            return t_exist;
        }
        ...
        {
            //add by chiuan
            bool t_exist = this._prefabs.TryGetValue(prefabName, out prefab);
            if(t_exist && prefab == null)
            {
                t_exist = false;
                prefab = null;
                _Remove(prefabName);
                ...
                #if UNITY_EDITOR
				Debug.LogError("SpawnPool TryGetValue is exist prefabName but transform is destroy."+"\n"+prefabName);
                #endif
            }
            return t_exist;
        }
