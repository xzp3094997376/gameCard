...
namespace ICSharpCode.SharpZipLib.Zip
{
    public class FastZip
    {
        ...
        {
            if (FileUtils.getInstance().isFileExist(zipFileName))
            {
                if (FileUtils.getInstance().isDirectoryExist(targetDirectory))
                {
                    FileUtils.getInstance().removeDirectory(targetDirectory);
                }
