...
namespace BestHTTP.JSON
{
	...
	public class Json
	{
		...
		{
            StringBuilder s_builder = new StringBuilder(BUILDER_CAPACITY);
            if (s_builder.Length > 0)
            {
                s_builder.Remove(0, s_builder.Length);
            }
            bool success = SerializeValue(json, s_builder);
			return (success ? s_builder.ToString() : null);
		}
        ...
        static StringBuilder s_sCon = new StringBuilder(BUILDER_CAPACITY);
        static int s_inSyn = 0;
        protected static string ParseString(char[] json, ref int index, ref bool success)
		{
            StringBuilder s_s;
            if (s_inSyn >= 1)
            {
                s_s = new StringBuilder(BUILDER_CAPACITY);
            }
            else
            {
                s_s = s_sCon;
				...
				if (c == '"') {
					...
					if (c == '"') {
						...
						if (remainingLength >= 4) {
                            ...
                            s_s.Append(Char.ConvertFromUtf32((int)codePoint));
							// skip 4 chars
							index += 4;
						} else {
							break;
