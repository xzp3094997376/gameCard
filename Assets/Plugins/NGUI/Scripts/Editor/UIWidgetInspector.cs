	...
	public void OnSceneGUI ()
	{
		...
		switch (type)
		{
			case EventType.Repaint:
			{
                if (isLines)
                {
                    NGUIHandles.DrawShadowedLine(handles, handles[0] - 2 * Vector3.up, handles[1] + 2 * Vector3.up, Color.red);
                    NGUIHandles.DrawShadowedLine(handles, handles[1] - 2 * Vector3.right, handles[2] + 2 * Vector3.right, Color.red);
                    NGUIHandles.DrawShadowedLine(handles, handles[2] - 2 * Vector3.up, handles[3] + 2 * Vector3.up, Color.red);
                    NGUIHandles.DrawShadowedLine(handles, handles[0] - 2 * Vector3.right, handles[3] + 2 * Vector3.right, Color.red);
                }
