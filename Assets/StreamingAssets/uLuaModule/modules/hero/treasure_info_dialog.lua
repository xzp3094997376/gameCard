...
function m:update(lua)
	self.treasure = lua.obj
	--  碎片
	self.suipian:CallUpdateWithArgs(lua)
	-- 模型
	if self.treasure:getType() == "treasurePiece" then 
		self.treasure = Treasure:new(self.treasure.Table.treasureId)
		--self.hero:LoadByModelId(self.char.Table.model_id, "idle", function() end, false, 0, 1)
	end 
	--self.hero:LoadByModelId(self.treasure.Table.model_id, "idle", function() end, false, 100, 1)
	self.txt_lv_name.text = self.treasure.itemColorName
end
...
function m:updateEquip()
	...
	self.txt_lv_name.text = self.treasure:getDisplayColorName()
	local magic = self.treasure:getMagic()[1]
	self.txt_attr_left.text = string.gsub("[ffff96]"..magic.format, "{0}", "[ffffff]" .. magic.arg)
	self.txt_qianhua.text = string.gsub("[ffff96]"..magic.format, "{0}", "[ffffff]" .. magic.arg2)
	self.txt_desc.text = self.treasure.desc
end 
