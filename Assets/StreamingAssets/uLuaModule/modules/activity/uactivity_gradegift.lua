...
function actList:Start(...)
    ...
    self._title=Tool.loadTopTitle(self.gameObject, TextMap.GetValue("Text_1_135"),function ()
...
function actList:update(data)
    ...
    if self._type=="recharge" and self._title~=nil then 
        self._title:CallTargetFunctionWithLuaTable("setTitle",TextMap.GetValue("Text_1_129"))
    end
end
...
function actList:getActivityData(...)
    self.list = {}
    if self._type==nil or self._type=="" then 
        self._type="recharge"
...
function actList:ApiReqActivityandFuli(result)
    ...
    self.binding:CallManyFrame(function()
        Events.Brocast('SelectActCallBack_grade', self._findIndex)
    end, 5)
...
function actList:showActivity(selected, id, go)
    ...
    if self.list[id].info == nil then
        ...
        if self.list[id].info.event == "totalCost" or self.list[id].info.event == "rankActivity" then--累计消耗切换页面没有刷新
              Api:getActivity(id,self._type, function(result)
                if result.ret == 0 then
                    self:getContentData(id,json.decode(result.info:toString()))
                    self:refreshContent(id,false)
...
function actList:refreshContent(id, ret)
    ...
    if not ret then 
        if self.bind[name_pre] ==nil then
            local index_pre = 1
            for k,v in pairs(self.bind) do
                if index_pre==5 and self:getTableLen()>=5 then 
    ...
    Api:getActGift(aid, gid, function(result)
        if result.drop ~= nil then
            ...
            if result.times~=nil and result.times>0 then
                ...
                if info.event=="lvlup" then 
            		info.status[gid].status= 2
            	else 
            		info.status[gid] = 2
            	end 
            end 
