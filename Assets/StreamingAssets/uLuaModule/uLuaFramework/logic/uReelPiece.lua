...
function M:updateInfo()
    self.info = Player.ReelPieceBagIndex[self.id]
...
function M:init(id, info)
    self.Table = TableReader:TableRowByID("reelPiece", id);
    if self.Table == nil then
        self.Table = TableReader:TableRowByUnique("rellPiece", "name", id)
    end
    if self.Table == nil then
        --    print("readTable err reelPiece" .. id)
        return
    end
