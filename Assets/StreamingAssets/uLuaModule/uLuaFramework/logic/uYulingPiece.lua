...
function M:updateInfo()
    local pieces = Player.yulingPieceBagIndex[self.id]
    self.info = pieces
...
function M:init(id, info)
    self.Table = TableReader:TableRowByID("yulingPiece", id);
    if self.Table == nil then
        self.Table = TableReader:TableRowByUnique("yulingPiece", "name", id)
    end
    if self.Table == nil then
        --     print("readTable err yulingPiece" .. id)
        return
    end
    self.typeIndex = 3;
    ...
    if drop~=nil and drop[0].type=="yuling" then 
        self._charTable = TableReader:TableRowByID("yuling", drop[0].arg)
    end
    if self._charTable == nil then
        self._charTable = TableReader:TableRowByID("yulingPiece", self.id)
    end
