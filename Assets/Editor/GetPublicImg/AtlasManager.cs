...
* 类 名 称：       AtlasManager 
* 命名空间：       
* 创建时间：       2014/7/28 9:30:20
* 作    者：       张晶晶
* 说    明：       图集预设管理包括创建，更新，删除
* 最后修改时间：   2014/7/30 
* 最后修 改 人：   张晶晶
* 曾修改人：
**********************************************************************/
...
public class AtlasManager : EditorWindow
{
    static public AtlasManager instance;

 //   public List<Texture> pTextures;
//    public string publicTexture;
...
//    public List<string> mDelNames = new List<string>();
    UIAtlas mLastAtlas;
   ...
   // void OnSelectionChange() { mDelNames.Clear(); Repaint(); }
    ...
    List<Texture> GetTextures(List<Texture> pTextures)
    {
        List<Texture> textures = new List<Texture>();
    //    string publicTexture = "Assets/_res/PublicImg";
        foreach(Texture tex in pTextures){
    ...
    /// <summary>
    /// Pack all of the specified sprites into a single texture,outer and inner rects of the sprites as needed.
    /// </summary>
    ...
    static public SpriteEntry DuplicateSprite(UIAtlas atlas, string spriteName)
    {
        ...
        if (se != null)
        {
            ...
            List<AtlasManager.SpriteEntry> sprites = new List<AtlasManager.SpriteEntry>();
            AtlasManager.ExtractSprites(atlas, sprites);
            sprites.Add(se);
            AtlasManager.UpdateAtlas(atlas, sprites);
   ...
   // Make the atlas texture readable
    ...
    static public bool UpdateTexture(UIAtlas atlas, List<SpriteEntry> sprites)
    {
        ...
        if (System.IO.File.Exists(newPath))
        {
            ...
            {
                MyDebug.LogError(newPath + " is not editable. Did you forget to do a check out?");
                return false;
            }
#endif
        ...
        if (PackTextures(tex, sprites))
        {
            ...
            if (newTexture)
            {
                if (tex == null) MyDebug.LogError("Failed to load the created atlas saved as " + newPath);
        ...
        else
        {
            ...
            //MyDebug.LogError("Operation canceled: The selected sprites can't fit into the atlas.\n" +
    ...
    /// 更新指定的图集预设
    /// </summary>
    public void updateAtlas(List<Texture> pTextures)
    {
        if (NGUISettings.atlas != null)
        {
            List<Texture> textures = GetTextures(pTextures);
    ...
    public void deleteSprite(List<string> mDelNames)
    {        
        List<SpriteEntry> sprites = new List<SpriteEntry>();
//		MyDebug.Log ("NGUI settion" + NGUISettings.atlas);
    ...
    /// <summary>
    /// 创建一个新的预设，根据指定的prefab路径来制作
    /// </summary>

    public void createNewAtlas(string publicAtlasPath, List<Texture> pTexture)
    {
        ...
        if (NGUISettings.atlas == null)
        {
            ...
            List<Texture> textures = GetTextures(pTexture);

            if (!string.IsNullOrEmpty(publicAtlasPath))
            {
                NGUISettings.currentPath = System.IO.Path.GetDirectoryName(publicAtlasPath);
                GameObject go = AssetDatabase.LoadAssetAtPath(publicAtlasPath, typeof(GameObject)) as GameObject;
                string matPath = publicAtlasPath.Replace(".prefab", ".mat");
                replace = true;
                ...
                Object prefab = (go != null) ? go : PrefabUtility.CreateEmptyPrefab(publicAtlasPath);

                // Create a new game object for the atlas
                string atlasName = publicAtlasPath.Replace(".prefab", "");
                atlasName = atlasName.Substring(publicAtlasPath.LastIndexOfAny(new char[] { '/', '\\' }) + 1);
                ...
                // Select the atlas
                go = AssetDatabase.LoadAssetAtPath(publicAtlasPath, typeof(GameObject)) as GameObject;
