...
public class HUDText : MonoBehaviour
{
	...
	public AnimationCurve offsetCurve = new AnimationCurve(new Keyframe[] { new Keyframe(0f, 0f), new Keyframe(2f, 40f) });
	...
	{
		mList.Remove(ent);
        //DestroyObject(ent.label.gameObject);
        ent.label.gameObject.transform.position = new Vector3(9999, 9999, 9999);
        mUnused.Add(ent);
	...
	void Update ()
	{
		...
		for (int i = mList.Count; i > 0; )
		{
			...
			offset += Mathf.Round((ent.label.cachedTransform.localScale.y - 1.0f) * ent.label.fontSize);
		}
        //fix bug can see something bug
        //for (int i = 0; i < mUnused.Count; i++)
        //{
        //    if(mUnused[i].label.gameObject.activeSelf)
        //        NGUITools.SetActive(mUnused[i].label.gameObject, false);
