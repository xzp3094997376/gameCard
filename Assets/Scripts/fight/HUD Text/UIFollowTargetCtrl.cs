...
public class UIFollowTargetCtrl : MonoBehaviour
{
	/// <summary>
   ...
   /// </summary>
    public Vector3 mOffset = new Vector3(0,1.5f,0);

    /// <summary>
    /// update objects depth when is need
    /// </summary>
    public bool mNeedUpdateDepth = false; 
    ...
    protected virtual void SetVisible(bool val)
	{
		mIsVisible = val;
	...
	void Update ()
	{
        ...
        bool isVisible = true;// (gameCamera.orthographic || pos.z > 0f) && (!disableIfInvisible || (pos.x > 0f && pos.x < 1f && pos.y > 0f && pos.y < 1f));
		...
		if (isVisible)
		{
            ...
            //if (mNeedUpdateDepth)
            //{
            //    float Distance = Vector3.Distance(target.position, gameCamera.transform.position);
            //    NGUITools.AdjustDepth();
            //}
		}
		OnUpdate(isVisible);
	}
