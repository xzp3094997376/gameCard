...
[ExecuteInEditMode]
[AddComponentMenu("NGUI/UI/NGUI Event System (MyUICamera)")]
...
public class MyUICamera : MonoBehaviour
{
    ...
    /// <summary>
    /// Camera type controls how raycasts are handled by the MyUICamera.
    /// </summary>
    ...
    static public BetterList<MyUICamera> list = new BetterList<MyUICamera>();
    ...
    /// Use MyUICamera.current to get the current camera.
    /// </summary>
    ...
    /// <summary>
    /// MyUICamera that sent out the event.
    /// </summary>

    static public MyUICamera current = null;
    ...
    static public GameObject selectedObject
    {
        ...
        set
        {
            SetSelection(value, MyUICamera.currentScheme);
        }
    }
    ...
    static protected void SetSelection(GameObject go, ControlScheme scheme)
    {
        ...
        else if (mCurrentSelection != go)
        {
            ...
            if (MyUICamera.list.size > 0)
            {
                MyUICamera cam = (mNextSelection != null) ? FindCameraForLayer(mNextSelection.layer) : MyUICamera.list[0];
    ...
    System.Collections.IEnumerator ChangeSelection()
    {
        ...
        if (mCurrentSelection != null)
        {
            current = this;
            currentCamera = mCam;
            MyUICamera.currentScheme = mNextScheme;
    ...
    static public MyUICamera eventHandler
    {
        get
        {
    ...
    static int CompareFunc(MyUICamera a, MyUICamera b)
    {
    ...
    static public bool Raycast2D(Vector3 inPos, out RaycastHit2D hit)
    {
        for (int i = 0; i < list.size; ++i)
        {
            ...
            if (cam.eventType == EventType.UI_3D)
            {
    ...
    {
        if (MyUICamera.currentScheme == MyUICamera.ControlScheme.Mouse)
            return (MyUICamera.hoveredObject == go);

        if (MyUICamera.currentScheme == MyUICamera.ControlScheme.Controller)
            return (MyUICamera.selectedObject == go);
    ...
    static public MyUICamera FindCameraForLayer(int layer)
    {
