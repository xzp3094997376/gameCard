...
public class DragDragPageComponent : MonoBehaviour
{
    ...
    public DragPageComponent scrollView;

    // Legacy functionality, kept for backwards compatibility. Use 'scrollView' instead.
    [HideInInspector]
    [SerializeField]
    DragPageComponent draggablePanel;

    Transform mTrans;
    DragPageComponent mScroll;
    ...
    void FindScrollView()
    {
        ...
        DragPageComponent sv = NGUITools.FindInParents<DragPageComponent>(mTrans);
    ...
    void OnPress(bool pressed)
    {
        ...
        if (scrollView && enabled && NGUITools.GetActive(gameObject))
        {
            ...
            {
                scrollView = NGUITools.FindInParents<DragPageComponent>(mTrans);
                mScroll = scrollView;
            }
        }
    }
