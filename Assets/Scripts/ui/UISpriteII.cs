...
/// 这个文件修改成 uisprite支持shader  通过MyShader做设置
/// </summary>

[ExecuteInEditMode]
[AddComponentMenu("NGUI/UI/NGUI SpriteII")]
public class UISpriteII : UIBasicSprite
{
    ...
    public override Material material
    {
        get
        {
            Material matBall = null;
            if (m_MyShader == null)
            {
                m_MyShader = Shader.Find("Unlit/Transparent Colored");
            }

            //auto create manager material
            matBall = MaterialManager.GetMaterial(mAtlas, m_MyShader.name);

            return (mAtlas != null) ? matBall : null;
    ...
    public UISpriteData GetAtlasSprite()
    {
        ...
        if (mSprite == null && mAtlas != null)
        {
            ...
            if (mSprite == null && mAtlas.spriteList.Count > 0)
            {
                ...
                if (mSprite == null)
                {
                    MyDebug.LogError(mAtlas.name + " seems to have a null sprite!");
                    return null;
                }
                mSpriteName = mSprite.name;
            }
